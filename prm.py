...
def sample_points2(sx, sy, gx, gy, rr, ox, oy, obstacle_kd_tree, rng):
    max_x = 10
    max_y = 9
    min_x = 0
    min_y = -9

    x_range = np.linspace(-5, 10, 20)
    y_range = np.linspace(-9, 9, 18)

    XX, YY = np.meshgrid(x_range, y_range)

    sample_xy = np.stack((XX.reshape(-1), YY.reshape(-1)),axis=-1)
    indices = np.linspace(0, sample_xy.shape[0]-1, sample_xy.shape[0])

...
class GMM2D(td.Distribution):
    ...
    def rsample(self, sample_shape=torch.Size(), output_mode=False):
        ...
        if output_mode is True:
            component_cat_samples = self.pis_cat_dist.sample(sample_shape)[...,:1]
        else:
        ...
        if torch.isnan(mvn_samples).sum():
            print()
        if output_mode is True:
            ...
            for n in range(bs):
                ...
                for t in range(time):
                    ...
                    search_grid = torch.stack(torch.meshgrid([torch.arange(x_min, x_max, 0.01),
                                                              torch.arange(y_min, y_max, 0.01)],indexing='ij'), dim=2

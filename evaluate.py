...
def visualize_distribution2d(ax,
    ...
    z = means[:, n, t, :, 2].min()
    XYZ = torch.meshgrid([torch.arange(x_min, x_max, 0.01), torch.arange(y_min, y_max, 0.01),torch.FloatTensor([z])],indexing='ijk')
...
def visualize_distribution2d_running(ax,
    ...
    y_max = y_range[1]
    for k in range(1):
        XYZ = torch.meshgrid([torch.arange(x_min+k*0.02*15, x_max+k*0.02*15, 0.05*15), torch.arange(y_min+k*0.02*15, y_max+k*0.02*15, 0.05*15),torch.FloatTensor([z])],indexing='ijk')
...
if __name__=='__main__':
    ...
    data_dict = load_data_cartesian('/home/pinhao/Desktop/franka_sim/data_bmi2d_w_goals_coi.json', 20, 20, test_size=0.1, aug=True)



    # trainData, testData, target_frequecy = load_data_cartesian2('/home/pinhao/Desktop/franka_sim/traj_fre20_noisy_100k.json', 10, 20, test_size=0.1, aug=True)
    # traindataset = ContextTrajDataset(data_dict["trainData"], data_dict["goals_train"], max_history_length=8, min_future_timesteps=12, eval=False)
    ...
    trajectron = Trajectron(hyperparams,
                            args.device)
    model = torch.load("checkpoints/RT_context_colavoid|bmi|ade95.15.pth")
